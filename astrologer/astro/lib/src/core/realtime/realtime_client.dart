...
/// Wraps the Centrifugo Dart client for the personal `user:{id}` and
/// `astro:{profile}` channels plus any room channels (`conv:` / `call:` / `chattyping:`) a screen asks for.
///
...
class RealtimeClient {
  ...
  Future<void> connect({required String userId, String? astroProfileId}) async {
    _wanted = true;
    final profile = (astroProfileId ?? '').isEmpty ? null : astroProfileId;
    if (_userId == userId &&
        _astroProfileId == profile &&
        (_client != null || _building)) {
      ensureConnected();
      return;
    }
    _astroProfileId = profile;
  ...
  Future<void> _rebuild(String userId) async {
    ...
    try {
      ...
      final profile = _astroProfileId;
      if (profile != null) _subscribePersonal(client, 'astro:$profile');
