...
/// Astrologer-side wiring for the shared chat engine.

/// Disk-backed [ChatOutbox] so a message sent while the app is killed isn't lost.
...
class DioChatTransport implements ChatTransport {
  ...
  Future<ChatPresence> presence() async {
    try {
      ...
      return ChatPresence.fromMap(res.data ?? const {}, iAmCustomer: false);
    } catch (_) {
...
class AstrologerChatIdentity implements ChatIdentity {
  AstrologerChatIdentity({required this.userId, required this.language});
  @override
  final String userId;
  @override
  final String language;
  @override
  ParticipantRole get role => ParticipantRole.astrologer;
  @override
  bool get canDictate => true;
}
